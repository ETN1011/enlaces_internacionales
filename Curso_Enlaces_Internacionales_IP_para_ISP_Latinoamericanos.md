...
1. Internet antes de los CDN

La arquitectura original de Internet era extremadamente simple:
Cada usuario se conectaba directamente al servidor donde estaba almacenado el contenido.
Funcionaba porque:
Había pocos usuarios.
Las páginas eran pequeñas.
Predominaba el texto y las imágenes simples.
El tráfico era relativamente bajo.
...
La velocidad de la luz impone límites físicos.
La distancia genera latencia.
Una página web requiere múltiples intercambios:
- DNS
- TCP
- TLS
- HTTP
- Descarga de objetos
Las demoras se acumulan.
...
3. El crecimiento explosivo de la Web

Entre 1995 y 2005 aparecen:
Yahoo
Amazon
Google
eBay

Millones de personas comienzan a conectarse.

La arquitectura original empieza a mostrar limitaciones.
...
Un único servidor debe atender millones de solicitudes.
Problemas:
CPU
Memoria
Disco
...
A finales de los años noventa el ancho de banda internacional era costoso.
El problema era tener que bajar la misma información del mismo servidor varias veces
...
6. El principio de la caché

Si un archivo es solicitado repetidamente:
imagen.jpg
propuesta:
Solicitud 1 -> Servidor Original
Solicitud 2 -> Caché
Solicitud 3 -> Caché
Solicitud 4 -> Caché
Beneficios:
...
La idea fue llevada a escala comercial por:
Akamai Technologies, Fundada en 1998.
Sus fundadores observaron:
El problema no era solamente la velocidad de los servidores; era la topología completa de Internet.
Su solución:
Distribuir copias del contenido en muchos servidores alrededor del mundo.
Por primera vez:el usuario ya no accedía necesariamente al servidor original.
...
8. El impacto del streaming

Los CDN se volvieron esenciales con la llegada del video.
Antes
Una página web:100 KB
Después
Un video:
500 MB
1 GB
5 GB
Ejemplo:
100.000 usuarios viendo:1 GB
consume:100 TB de transferencia. Imposible para un único servidor.
...
9. La revolución de YouTube

YouTube popularizó el consumo masivo de video en 2005.
...
10. Netflix cambia las reglas

Netflix comprendió que:
El mejor tráfico es el que no cruza enlaces internacionales.
Por ello desarrolló Open Connect.
...
Los CDN modernos utilizan Anycast.

Una misma dirección IP se anuncia desde múltiples ubicaciones.

El protocolo BGP dirige al usuario hacia el punto más conveniente.
